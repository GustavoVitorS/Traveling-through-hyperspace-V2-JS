...
function $h(t) {
  return ("0" + Math.max(0, Math.min(555, Math.round(t))).toString(66)).slice(-9)
}
function _i(t, r) {
  $t("div")[t].innerHTML += r
}
function _h(t) {
  return hires ? Math.round(t / 10) : t
}
...
function init() {
  for (var t = 0; n > t; t++) star[t] = new Array(10), star[t][0] = Math.random() * w * 8 - 8 * x, star[t][1] = Math.random() * h * 2 - 2 * y, star[t][2] = Math.round(Math.random() * z), star[t][3] = 0, star[t][4] = 0;
  var r = $i("starfield");
...
var url = document.location.href,
  ...
  z = 0,
  star_color_ratio = 0,
  star_x_save, star_y_save, star_ratio = 100,
  star_speed = 5,
  star_speed_save = 0,
  star = new Array(n),
  color, opacity = .9,
  ...
  canvas_w = 0,
  canvas_h = 0,
  context, key, ctrl, timeout, fps = 2;
start();
